 ...
 * Version: 2.1.2 - FIXED sprintf format bug
 *
...
// FIXED: Build width string separately to avoid sprintf format issues
$width_value = $device === 'desktop' ? $banner_width . 'px' : '100%';

// Inline CSS variables - FIXED FORMAT
$inline_styles = sprintf(
    ...
    esc_attr($text_color),
    esc_attr($width_value), // FIXED: Pass as pre-formatted string
    $banner_padding,
    $font_size_headline,
    $font_size_desc,
    $font_size_button,
    ...
    <div class="signalkit-banner-content">
        ...
        <div class="signalkit-icon" role="img" aria-label="<?php esc_attr_e('Google News Icon', 'signalkit-for-google'); ?>">
            <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">
                ...
                <path d="M24 14C18.48 14 14 18.48 14 24C14 29.52 18.48 34 24 34C29.52 34 34 29.52 34 24C34 18.48 29.52 14 24 14ZM24 30C20.69 30 18 27.31 18 24C18 20.69 20.69 18 24 18C27.31 18 30 20.69 30 24C30 27.31 27.31 30 24 30Z" fill="<?php echo esc_attr($secondary_color); ?>"/>
                <circle cx="24" cy="24" r="4" fill="<?php echo esc_attr($secondary_color); ?>"/>
            </svg>
        </div>
        ...
        <div class="signalkit-text">
            <?php if (!empty($headline)): ?>
                <h3 id="<?php echo esc_attr($banner_id); ?>-headline" class="signalkit-headline" style="font-size: <?php echo esc_attr($font_size_headline); ?>px; color: <?php echo esc_attr($text_color); ?>;">
                    <?php echo wp_kses_post($headline); ?>
                </h3>
            <?php endif; ?>

            <?php if (!empty($description)): ?>
                <p id="<?php echo esc_attr($banner_id); ?>-description" class="signalkit-description" style="font-size: <?php echo esc_attr($font_size_desc); ?>px; color: <?php echo esc_attr($text_color); ?>;">
        ...
        <div class="signalkit-actions">
            <?php if (!empty($google_news_url)): ?>
                <a href="<?php echo esc_url($google_news_url); ?>"
                   ...
                   style="font-size: <?php echo esc_attr($font_size_button); ?>px; background-color: <?php echo esc_attr($primary_color); ?>; color: <?php echo esc_attr($secondary_color); ?>;"
                   target="_blank"
