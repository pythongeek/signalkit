...
class SignalKit_Admin {
    ...
    public function sanitize_settings($input) {
        ...
        if (isset($input['analytics_tracking'])) {
            $sanitized['analytics_tracking'] = $input['analytics_tracking'] ? 1 : 0;
        }
        if (isset($input['enable_rate_limiting'])) {
            $sanitized['enable_rate_limiting'] = $input['enable_rate_limiting'] ? 1 : 0;
        }
        if (isset($input['enable_csp'])) {
            $sanitized['enable_csp'] = $input['enable_csp'] ? 1 : 0;
        }
...
";
            ...
            wp_send_json_success([
                ...
                'device' => $device,
                'debug' => [
                    'settings_applied' => count($sanitized),
                    'template_used' => 'banner-' . $banner_type . '.php'
                ]
            ]);
        ...
        if (!empty($settings['follow_enabled']) && !empty($settings['preferred_enabled'])) {
            ...
            if ($follow_mobile && $preferred_mobile) {
                ...
                if ($follow_order === $preferred_order) {
                    echo '<div class="notice notice-warning is-dismissible"><p><strong>SignalKit:</strong> Both banners have the same mobile stack order and will overlap. <a href="' . admin_url('admin.php?page=signalkit-for-google') . '">Fix now</a></p></div>';
                }
            }
        }
    }
