...
class SignalKit_Activator {
    ...
    public static function activate() {
        ...
        $new_version = defined('SIGNALKIT_VERSION') ? SIGNALKIT_VERSION : '2.0.0';
        ...
        if ($is_upgrade) {
            self::run_upgrade_routines($old_version, $new_version);
        }
     ...
     */
    private static function run_upgrade_routines($old_version, $new_version) {
        self::safe_log('Activator: Running upgrade routines', array(
            'from' => $old_version,
            'to' => $new_version
        ));
    ...
    private static function upgrade_to_2_0() {
        self::safe_log('Activator: Running 2.0 upgrade routine');
        ...
        if ($updated) {
            update_option('signalkit_settings', $settings);
            self::safe_log('Activator: 2.0 upgrade completed - new fields added');
        }
        
        update_option('signalkit_upgraded_to_2_0', true, 'yes');
    }
    ...
    public static function check_requirements() {
        ...
        if (version_compare($wp_version, '5.0', '<')) {
            return new WP_Error(
                'wp_version_too_low',
                sprintf(
                    __('SignalKit for Google requires WordPress 5.0 or higher. You are running version %s.', 'signalkit-for-google'),
                    $wp_version
                )
            );
        }
        ...
        if (version_compare(PHP_VERSION, '7.2', '<')) {
            return new WP_Error(
                'php_version_too_low',
                sprintf(
                    __('SignalKit for Google requires PHP 7.2 or higher. You are running version %s.', 'signalkit-for-google'),
                    PHP_VERSION
                )
            );
        }
